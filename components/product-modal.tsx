...
import { Star, Package, TrendingDown, Clock, ExternalLink } from 'lucide-react'
...
export default function ProductModal({ product, onClose }: ProductModalProps) {
  ...
  const averagePrice = (product.priceHistory.reduce((a, b) => a + b, 0) / product.priceHistory.length).toFixed(2)
    ...
    <Dialog open={true} onOpenChange={onClose}>
      <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="grid md:grid-cols-2 gap-6 mt-4">
          ...
          <div className="space-y-4">
            <div>
              ...
              <p className="text-lg font-semibold text-foreground">{product.competitor}</p>
            </div>
            ...
            <div className="flex items-center gap-4">
              <Badge variant="secondary" className={`${stockColor[product.stock]}`}>
                <Package className="h-4 w-4 mr-2" />
                {product.stock}
              </Badge>
              <div className="flex items-center gap-1">
                <Star className="h-5 w-5 fill-yellow-500 text-yellow-500" />
                <span className="font-semibold text-foreground text-lg">{product.rating}</span>
        ...
        <div className="mt-6 space-y-4">
          <div>
            <h3 className="font-semibold text-lg text-foreground mb-4">Price History</h3>
            <div className="grid grid-cols-2 gap-4 mb-4">
              <div className="p-4 border border-border rounded-lg">
                <p className="text-sm text-muted-foreground">Average Price</p>
                <p className="text-2xl font-bold text-foreground mt-1">${averagePrice}</p>
              </div>
              <div className="p-4 border border-border rounded-lg">
                <p className="text-sm text-muted-foreground">Lowest Price</p>
                <p className="text-2xl font-bold text-green-500 mt-1">${lowestPrice}</p>
              </div>
            </div>
          ...
          {product.isDiscounted && (
            <div className="p-4 border border-green-500/20 bg-green-500/10 rounded-lg">
              ...
              <p className="text-xs text-muted-foreground mt-1">
                This product is currently on sale. Price may change without notice.
              </p>
            </div>
          )}
        </div>
